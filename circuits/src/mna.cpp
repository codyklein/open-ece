...
void check_topology(const Circuit& circuit) {
    ...
    for (const auto& part : d.components)
        if (const auto* v = std::get_if<VoltageSource>(&part)) {
            ...
            std::vector<double> drop(count), magnitude(count);
            queue = {p};
            parent[p] = p;
            for (std::size_t i = 0; i < queue.size(); ++i)
            ...
            if (parent[n] != count) {
                ...
                const double tolerance = policy::constraint_loop_factor *
                                         ...
                                         (magnitude[n] + std::abs(v->voltage_volts));
                if (std::abs(drop[n] - v->voltage_volts) > tolerance)
                    throw CircuitError(ErrorCode::contradictory_voltage_constraint,
                                       "Contradictory ideal-voltage-source loop.", {}, loop);
                if (redundant.empty())
                    redundant = loop;
            } else {
                forest[p].push_back({n, v->voltage_volts, v->id});
                forest[n].push_back({p, -v->voltage_volts, v->id});
            }
        }
    if (!redundant.empty())
...
MnaSystem assemble(const Circuit& circuit) {
    ...
    auto add = [&](std::size_t i, std::size_t j, double value) { s.matrix[i * k + j] += value; };
        ...
        std::visit(
            [&](const auto& c) {
                ...
                if constexpr (std::is_same_v<T, Resistor>) {
                    ...
                    if (!pg)
                        s.rhs[p] -= c.current_amperes;
                    if (!ng)
                        s.rhs[n] += c.current_amperes;
                } else {
